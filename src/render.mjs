...
export const variants = {
  ...
  minecraft: {
    ...
    meta: "a miner breaks contribution ore blocks",
    marker: "",
    trackDash: "4 7",
    sparkShape: "diamond",
  },
  hash: {
    label: "Hash Cracker",
    meta: "commit hashes decrypt as activity rises",
    marker: "",
    trackDash: "2 5",
    sparkShape: "line",
  },
  ...
  skyline: {
    label: "City Skyline",
    meta: "weeks become buildings with lit commit windows",
    marker: "",
    trackDash: "2 6",
    sparkShape: "circle",
  },
  constellation: {
    label: "Constellation",
    meta: "commit days become connected stars",
    marker: "",
    trackDash: "2 8",
    sparkShape: "star",
  },
  sketch: {
    label: "Etch-a-Sketch",
    meta: "a single line draws the contribution history",
    marker: "",
    trackDash: "7 5",
    sparkShape: "circle",
  },
...
export function renderContributionRunner({ username: login, weeks, title, theme, variant = "runner" }) {
  ...
  if (variant === "minecraft") {
    return renderMinecraftMiner({ login, days, total, max, label });
  }

  if (variant === "hash") {
    return renderHashCracker({ login, days, total, max, label });
  }

  if (variant === "pipeline") {
    return renderBuildPipeline({ login, days, total, max, label });
  }

  if (variant === "skyline") {
    return renderCitySkyline({ login, days, total, max, label });
  }

  if (variant === "constellation") {
    return renderConstellation({ login, days, total, max, label });
  }

  if (variant === "sketch") {
    return renderEtchSketch({ login, days, total, max, label });
  }
...
const MODEL_CELL = 11;
const MODEL_GAP = 2;
const MODEL_GRID_X = 24;
const MODEL_GRID_Y = 48;

function renderMinecraftMiner({ login, days, total, max, label }) {
  const cells = modelCells(days, max);
  const blocks = cells.map((cell, index) => {
    const x = MODEL_GRID_X + cell.week * (MODEL_CELL + MODEL_GAP);
    const y = MODEL_GRID_Y + cell.day * (MODEL_CELL + MODEL_GAP);
    const delay = fmt((index % 90) * 0.08);
    return `<g>
      <rect x="${x}" y="${y}" width="${MODEL_CELL}" height="${MODEL_CELL}" fill="${minecraftColor(cell.level)}" stroke="#000" stroke-width=".8"/>
      <path d="M${x} ${y + 3}h${MODEL_CELL}M${x + 3} ${y}v${MODEL_CELL}" stroke="#000" stroke-opacity=".18"/>
      ${cell.level ? `<rect x="${x}" y="${y}" width="${MODEL_CELL}" height="${MODEL_CELL}" fill="#fff" opacity="0"><animate attributeName="opacity" values="0;.35;0" dur=".5s" begin="${delay}s" repeatCount="indefinite"/></rect>` : ""}
    </g>`;
  }).join("");
  const path = activePath(cells, MODEL_GRID_X + 5.5, MODEL_GRID_Y + 5.5);
  return modelSvg(label, login, total, "Minecraft-style miner breaks contribution ore blocks", `
    <defs><linearGradient id="sky" x1="0" y1="0" x2="0" y2="1"><stop stop-color="#7ec0ee"/><stop offset="1" stop-color="#1a3a5c"/></linearGradient></defs>
    <rect width="100%" height="100%" fill="url(#sky)"/>
    <rect x="0" y="150" width="${MODEL_W}" height="50" fill="#6b4423"/>
    <rect x="0" y="143" width="${MODEL_W}" height="10" fill="#5a9943"/>
    ${modelTitle(label, total, "#f5f5f5", "Code Miner")}
    ${blocks}
    <path d="${path}" fill="none" stroke="#facc15" stroke-width="2" stroke-dasharray="4 8" opacity=".75"/>
    <g transform="translate(-7 -13)">
      <animateMotion dur="18s" repeatCount="indefinite" rotate="auto" path="${path}"/>
      <rect x="3" y="7" width="11" height="11" fill="#2563eb" stroke="#111"/>
      <rect x="5" y="0" width="7" height="7" fill="#f2c078" stroke="#111"/>
      <path d="M13 2 L22 9 M20 7 L16 11" stroke="#e5e7eb" stroke-width="2"/>
    </g>
  `);
}

function renderHashCracker({ login, days, total, max, label }) {
  const cells = modelCells(days, max);
  const rows = Array.from({ length: 9 }, (_, row) => {
    const y = 45 + row * 14;
    const text = hashLine(days, row);
    return `<text x="24" y="${y}" fill="#39d353" font-size="10" opacity="${row === 4 ? 1 : .55}">${text}
      <animate attributeName="opacity" values=".35;1;.35" dur="${2 + row * .15}s" repeatCount="indefinite"/>
    </text>`;
  }).join("");
  const hot = cells.filter((cell) => cell.level > 2).slice(0, 45).map((cell, index) => {
    const x = MODEL_GRID_X + cell.week * 15;
    const y = 42 + cell.day * 18;
    return `<rect x="${x}" y="${y}" width="9" height="12" fill="#39d353" opacity="0">
      <animate attributeName="opacity" values="0;.8;0" dur="1.3s" begin="${fmt(index * .12)}s" repeatCount="indefinite"/>
    </rect>`;
  }).join("");
  return modelSvg(label, login, total, "terminal hash stream decrypts active contribution days", `
    <rect width="100%" height="100%" fill="#050807"/>
    ${modelTitle(label, total, "#39d353", "Hash Cracker")}
    <rect x="18" y="32" width="824" height="142" rx="6" fill="#020403" stroke="#1f6f3a"/>
    ${rows}
    ${hot}
    <text x="24" y="186" fill="#8b949e" font-size="9">ACCESS: ${escapeXml(login)} / CONTRIBUTIONS: ${total} / STATUS: DECRYPTING</text>
  `);
}

function renderBuildPipeline({ login, days, total, max, label }) {
  const cells = modelCells(days, max);
  const weeks = Array.from({ length: MODEL_COLS }, (_, week) => {
    const items = cells.filter((cell) => cell.week === week);
    const sum = items.reduce((acc, cell) => acc + cell.count, 0);
    const level = invaderLevel(sum, Math.max(1, max * MODEL_ROWS));
    return { week, level, sum };
  }).filter((week) => week.sum > 0).slice(0, 48);
  const stages = ["COMMIT", "TEST", "BUILD", "SHIP"];
  const stageNodes = stages.map((stage, index) => {
    const x = 110 + index * 190;
    return `<g><rect x="${x - 44}" y="78" width="88" height="44" rx="6" fill="#161b22" stroke="#30363d"/>
      <text x="${x}" y="104" text-anchor="middle" font-size="10" fill="#c9d1d9">${stage}</text></g>`;
  }).join("");
  const commits = weeks.map((week, index) => {
    const delay = fmt(index * .35);
    const color = ["#0e4429", "#006d32", "#26a641", "#39d353"][Math.max(0, week.level - 1)];
    return `<g opacity="0">
      <animate attributeName="opacity" values="0;1;1;0" dur="8s" begin="${delay}s" repeatCount="indefinite"/>
      <circle r="8" fill="${color}" stroke="#fff" stroke-opacity=".25">
        <animateMotion dur="8s" begin="${delay}s" repeatCount="indefinite" path="M70 100 C180 40 260 160 350 100 S540 40 640 100 S750 145 810 100"/>
      </circle>
    </g>`;
  }).join("");
  return modelSvg(label, login, total, "commits move through a CI pipeline", `
    <rect width="100%" height="100%" fill="#0d1117"/>
    ${modelTitle(label, total, "#c9d1d9", "Build Pipeline")}
    <path d="M70 100 C180 40 260 160 350 100 S540 40 640 100 S750 145 810 100" fill="none" stroke="#30363d" stroke-width="8" stroke-linecap="round"/>
    ${stageNodes}
    ${commits}
  `);
}

function renderCitySkyline({ login, days, total, max, label }) {
  const cells = modelCells(days, max);
  const buildings = Array.from({ length: MODEL_COLS }, (_, week) => {
    const items = cells.filter((cell) => cell.week === week);
    const sum = items.reduce((acc, cell) => acc + cell.count, 0);
    const height = Math.max(10, Math.min(128, 10 + sum * 3));
    const x = 20 + week * 16;
    const y = 172 - height;
    const lit = items.reduce((acc, cell) => acc + cell.level, 0);
    return `<g>
      <rect x="${x}" y="${y}" width="12" height="${height}" fill="${week % 2 ? "#243447" : "#1b2838"}" stroke="#30445f"/>
      ${Array.from({ length: Math.min(7, Math.floor(height / 13)) }, (_, row) => {
        const wy = y + 8 + row * 12;
        const on = row < lit % 8;
        return `<rect x="${x + 3}" y="${wy}" width="2" height="3" fill="${on ? "#facc15" : "#475569"}"/><rect x="${x + 7}" y="${wy}" width="2" height="3" fill="${on && row % 2 ? "#facc15" : "#475569"}"/>`;
      }).join("")}
    </g>`;
  }).join("");
  return modelSvg(label, login, total, "weekly contributions become a living skyline", `
    <defs><linearGradient id="night" x1="0" y1="0" x2="0" y2="1"><stop stop-color="#061826"/><stop offset="1" stop-color="#111827"/></linearGradient></defs>
    <rect width="100%" height="100%" fill="url(#night)"/>
    ${modelTitle(label, total, "#e5e7eb", "City Skyline")}
    <circle cx="760" cy="48" r="18" fill="#fde68a" opacity=".85"/>
    ${buildings}
    <rect x="0" y="172" width="${MODEL_W}" height="28" fill="#020617"/>
  `);
}

function renderConstellation({ login, days, total, max, label }) {
  const cells = modelCells(days, max).filter((cell) => cell.level > 0);
  const points = cells.slice(0, 120).map((cell) => ({
    x: 35 + cell.week * 15,
    y: 42 + cell.day * 18 + ((cell.week * 7) % 6),
    level: cell.level,
  }));
  const lines = points.slice(1).map((point, index) => {
    const prev = points[index];
    return `<path d="M${prev.x} ${prev.y}L${point.x} ${point.y}" stroke="#60a5fa" stroke-opacity=".24" stroke-width="1"/>`;
  }).join("");
  const stars = points.map((point, index) => `<circle cx="${point.x}" cy="${point.y}" r="${1.4 + point.level}" fill="#dbeafe">
    <animate attributeName="opacity" values=".35;1;.35" dur="${fmt(2 + (index % 9) * .2)}s" repeatCount="indefinite"/>
  </circle>`).join("");
  return modelSvg(label, login, total, "active days form a generated constellation", `
    <rect width="100%" height="100%" fill="#020617"/>
    ${modelTitle(label, total, "#dbeafe", "Constellation")}
    ${lines}${stars}
    <text x="28" y="184" fill="#93c5fd" font-size="9" letter-spacing="3">URSA COMMITTUS / ${escapeXml(login)}</text>
  `);
}

function renderEtchSketch({ login, days, total, max, label }) {
  const cells = modelCells(days, max).filter((cell) => cell.level > 0);
  const points = cells.map((cell) => `${35 + cell.week * 15},${45 + cell.day * 17}`);
  const path = points.length ? `M${points.join(" L")}` : "M35,100 L825,100";
  return modelSvg(label, login, total, "a red toy screen draws the contribution path", `
    <rect width="100%" height="100%" fill="#b91c1c"/>
    <rect x="42" y="32" width="776" height="130" rx="10" fill="#dbe3e8" stroke="#7f1d1d" stroke-width="5"/>
    <text x="430" y="24" text-anchor="middle" fill="#fee2e2" font-size="12" font-weight="700" letter-spacing="5">${escapeXml(label).toUpperCase()}</text>
    <path d="${path}" fill="none" stroke="#334155" stroke-width="2.2" stroke-linejoin="round" stroke-linecap="round" stroke-dasharray="1300" stroke-dashoffset="1300">
      <animate attributeName="stroke-dashoffset" values="1300;0;0;1300" keyTimes="0;.6;.85;1" dur="16s" repeatCount="indefinite"/>
    </path>
    <circle cx="74" cy="178" r="16" fill="#f8fafc" stroke="#7f1d1d" stroke-width="3"/>
    <circle cx="786" cy="178" r="16" fill="#f8fafc" stroke="#7f1d1d" stroke-width="3"/>
    <text x="430" y="184" text-anchor="middle" fill="#fee2e2" font-size="9">${total} contributions</text>
  `);
}

function modelSvg(label, login, total, desc, body) {
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${MODEL_W}" height="${MODEL_H}" viewBox="0 0 ${MODEL_W} ${MODEL_H}" role="img" aria-labelledby="title desc">
    <title id="title">${escapeXml(label)} contribution animation</title>
    <desc id="desc">${escapeXml(desc)} for ${escapeXml(login)} with ${total} contributions.</desc>
    <style>text{font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,monospace}.mono{font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,monospace}</style>
    ${body}
  </svg>`;
}

function modelTitle(label, total, color, sublabel) {
  return `<text x="20" y="20" fill="${color}" font-size="13" font-weight="700" letter-spacing="3">${escapeXml(label).toUpperCase()}</text>
    <text x="20" y="36" fill="${color}" opacity=".7" font-size="9">${escapeXml(sublabel)} - ${total} contributions</text>`;
}

function modelCells(days, max) {
  return days.map((day) => ({
    week: day.weekIndex,
    day: day.dayIndex,
    count: day.contributionCount,
    level: invaderLevel(day.contributionCount, max),
  }));
}

function activePath(cells, offsetX, offsetY) {
  const points = cells
    .filter((cell) => cell.level > 0)
    .slice(0, 140)
    .map((cell) => `${offsetX + cell.week * (MODEL_CELL + MODEL_GAP)} ${offsetY + cell.day * (MODEL_CELL + MODEL_GAP)}`);
  return points.length ? `M${points.join(" L")}` : `M${offsetX} ${offsetY} L${MODEL_W - offsetX} ${offsetY}`;
}

function minecraftColor(level) {
  return ["#2a2226", "#5a5a5a", "#6b4423", "#2563eb", "#22c55e"][level] || "#2a2226";
}

function hashLine(days, row) {
  const alphabet = "0123456789abcdef";
  let line = "";
  for (let index = 0; index < 78; index += 1) {
    const day = days[(index * 7 + row * 13) % days.length];
    line += alphabet[(day.contributionCount + index + row) % alphabet.length];
  }
  return line;
}
